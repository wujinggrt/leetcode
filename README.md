...
分析：n x n 棋盘中，横、竖、斜对角线和反斜对角线只能放置一个皇后。

解决思路：Dfs + 回溯。及时剪枝，节省时间。从第一行开始，在每个列遍历地放置皇后，再递归地遍历第二行、三行等。使用三个 set 记录列、斜对角线和反斜对角线的皇后放置情况，判断是否发生冲突。遍历每行时，尝试在此行的每个位置放置皇后。判断此位置的同列、同两斜对角线，如果发生冲突，则在下一列尝试放置；不冲突则放置，把此为止所在的列、两对角线标记，提供给下轮递归的判断。

表示方式：
- 放置即在棋盘上设置一个 flag。
- 列、两对角线的表示：
    - 使用一个数字可以表示一列被占有
    - 斜对角线的表示稍微复杂。斜对角线上的元素坐标为 i, j，下一个元素的坐标为 i+1, j+1；反斜对角线的情况，把 +1 替换为 -1 即可。斜对角线上元素坐标只差相等，不同斜坐标则拥有不同的差值。比如，(0,0)，(1,0) 所在斜对角线坐标差值分别为 0 和 1。
    - 反斜对角线同理。但是使用两坐标轴位置之和来表示。

遍历到最后一行，则可以生成棋盘。

### 位运算技巧

`x & (-x)` 表示仅保留 x 二进制中**最低位 1**。-x为x取反加1，也就是相较于x的最低位1及之后的所有位, x与-x相同，所有更高的位都有-x对应位与其相反。所以x&(-x)操作将最低为1的更高位和更低位都得到了0，剩余最低为1.

`x & (x - 1)` 将二进制中**最低位 1 设置 0**。x-1仅仅翻转了x的最低位1及之后的0位01111…1，因为01111…1 + 1 = 10000…0，等于最低位1及其后面部分。x - 1 数学含义是从 x 最低位 1 借位，减去 1。

### 使用位运算优化

优化存储空间和运算。用位运算记录皇后位置，减少存储空间，加速查找。使用 3 个至少 N 位的整数记录已放置的列和两对角线。使用 0 表示无皇后，1 表示有皇后。
- 列：选中的列号放置皇后，则在对应位置 1。棋盘最左列（0 列）对应最低二进制位。当遍历到下一行时，如果记录的 cols 左移 1 位，恰好代表对角线。
- 对角线：两对角线上不能有冲突，计算方式和表达方式比较技巧化。可以通过观察样例，发现 **insights**，再总结规律。这样的方式会更容易理解。

例子，以 0 起始，0 和 1 行的 2 4 列放置了皇后，遍历到第 2 行时，有：
- columns = 0b00010100，代表第 2 和 4 列不能放置皇后
- 对角线 diagonals1 = 0b00110000，代表 2 行的 4 和 5 列不能放置皇后。其中第 0 行 2 列和 1 行 4 列对角线的延伸。
- 反对角线 diagonals2 = 0b00001001，代表 2 行的 0 和 3 列不能放置皇后。分别对应 0 行 2 列和 1 行 4 列反对角线延伸。

洞察对角线的计算规律：在 3 行时，只需要左移原来 2 行的对角线位置记录，即可获得在 3 行的对角线位置记录。反斜对角线右移即可。具体运算如下：
- 初始时，没有放置皇后，三个记录都为 0
- 在当前行放置皇后到 i 列，在三个整数的第 i 二进制位上置 1
- 进入下一行，columns 保持不变，对角线 diagonals 左移 1 位，antiDiagonals 右移 1 位。这样得到了皇后所在位置信息，1 的部分不能放置，0 则可以

计算可以放置位置：$(2^n - 1) & (~(columns | diagonals1 | antiDiagonals))$ 中，结果为 1 的部分可以放置皇后，遍历这些位置即可。取反部分，将 1 从占位的含义转变为可以放置。

## 53 最大子数组和: 细节

求能够获得和最大的子数组。如果一个子数组的和是负数，那么与它结合的相邻子数组之和一定会减少。使用一个数字记录遍历的累加之和，如果遇到负数，重置为 0，代表重新开始记录。

## 54 螺旋矩阵: 细节，AOE，AOV 思路

关键找到遍历时方向。右，下，左，上，右，...，如此循环。并且每次遍历时，行、列下次会少一个元素。有两个思路：
- 类似图的事件在边的思路，关注调整步进方向和步进步数的规律
- 类似图的事件在顶点思路，关注遍历顶点数量和顶点方向的规律
...
每个方向移动步数：右 3，下 5，左 3，上 4，右 2，下 3，左 1，上 2

### 关注步进方向和步数

初始时，在第一个元素，纳入结果后，开始决定下一步方向。

决定方向：按照右下左上右...顺序，依次遍历。初始时，水平步数应当为列 - 1，垂直步数应当为行 - 1。随后，每遍历完水平或垂直方向，都要减少水平或垂直的步数。

特殊处理：在初始时，左上角的元素要想象为第一个内圈。需要假想其左侧有一个虚拟的顶点，由它遍历过来。所以，第一轮有所不同，步进步数为列的数量，并且已经由假想的第一个元素步进过来。
...
每次移动时，遍历完当前行、列方向的元素。每次换方向时，由于终点位置占用了下一轮方向的首个顶点，所以下一轮方向遍历的顶点要减少一个。比如水平遍历 4 个顶点，在向下遍历完后，下一轮水平的顶点只有 3 个。

## 55 Jump Game: 细节

数组中的数字决定所在位置最大跳跃距离，判断是否能跳出数组。

分析：发现数字 0 之后，说明此点需要越过。如果不能前进

思路：按顺序遍历数组，根据当前数字，还有历史记录，计算最远距离。遇到 0，则判断此位置是否截至目前的最远距离，若是，则只能止步；否则跳过即可。

## 56 融合区间：分析细节

写好判断融合的函数。首先排序，排序后的区间序列，按照区间下限的升序排列，下限相同时，再比较上限。随后，依次遍历区间，如果区间相交，记录的区间只需要扩充上限。若不相交，说明当前已是最大区间，纳入新的区间即可。

## 57 插入区间：查找插入位置的细节

给定的区间是有序的，可以使用二分查找。
1. 根据区间上限，二分查找第一个**上限**不小于（lower_bound() 查找，即 >=）**待插入区间下限**的位置，此位置是待插入区间融入的左侧。
2. 再二分查找，找到第一个**下限**大于待插入区间上限。此区间的左侧是待插入区间可能融入的位置。

上面步骤 1 和 2 确定了插入的区间，并且是左闭右开的。所以可以调整，单独取出这些区间（可能大于 1 个），比较 left 的下限和 right - 1 上限来调整。
...
根据 int n 生成 nxn 螺旋矩阵。根据螺旋数组的规律，遍历数组并填充。可以简化问题，使用 0 作为边界，每次直接按照一个方向填充，遇到 0 则变换方向填充。

为了方便表达方向，可以使用一个偏移数组表示更新的方向。
...
Trick：直接修改 board，但是不稳健，不建议。

使用 BFS 遍历上下左右位置，发现相邻的，继续遍历，直到找到所有满足的，返回 true；发现找不到，重新尝试别的起点。

遍历所有顶点式耗时的，所以预处理，提取可能匹配的顶点。仅对出现首字符的位置 BFS。还可以比较首尾字符，进一步减少搜索起点，如果尾字符更少，那么取反向的单词即可。
...
如果需要记录遍历了哪一层，可以使用 deque 记录每层节点，依次遍历。比如，第一个顶点是 0 层，root。首先记录当前的 size，遍历当前层的所有节点，添加各自所有的孩子节点，再移除当前层所有节点。便可一层一层地遍历。或者给各个节点打上层的标签，也可实现层序记录。
