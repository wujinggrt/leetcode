...
在未排序数组，找到第一个缺失正整数，要求时间复杂度 O(n)，空间复杂度 O(1)。不能够选择排序和哈希的策略。所以只能折中，原地修改数组，实现类似哈希表的功能。长度为 n 的数组，缺失正数在 [1,n+1]。遍历数组，如果当前数为 x，那么就在 arr[x-1] 处打上标记（下标要减 1 作为 offset）。如果最后，所有元素都打上标记，那么结果就是 n+1，否则就是第一个未打标记的数。如果标记处 arr[x-1] 有数，那么将其往后移动，直到找到一个没有标记的数。
